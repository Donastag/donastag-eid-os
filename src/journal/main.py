import os
import json
import logging
from typing import Any
import asyncpg
from fastapi import FastAPI, HTTPException
from pydantic import BaseModel

logging.basicConfig(level=logging.INFO)
logger = logging.getLogger(__name__)
app = FastAPI(title="Engineering Journal", version="0.1.0")

PG_DSN = os.getenv(
    "PG_DSN",
    f"postgresql://{os.getenv('POSTGRES_USER', 'donastag')}:{os.getenv('POSTGRES_PASSWORD', 'postgres')}@{os.getenv('POSTGRES_HOST', 'postgres')}:{os.getenv('POSTGRES_PORT', '5432')}/{os.getenv('POSTGRES_DB', 'donastag')}",
)


class JournalEntry(BaseModel):
    source: str
    title: str
    body: str
    tags: list[str] | None = None
    metadata: dict[str, Any] | None = None


@app.post("/entries")
async def create_entry(entry: JournalEntry):
    conn = await asyncpg.connect(PG_DSN)
    try:
        row = await conn.fetchrow(
            "INSERT INTO journal_entries (source, title, body, tags, metadata) VALUES ($1, $2, $3, $4, $5) RETURNING id, source, title, body, tags, metadata, created_at, updated_at",
            entry.source,
            entry.title,
            entry.body,
            entry.tags or [],
            json.dumps(entry.metadata or {}),
        )
        return dict(row)
    finally:
        await conn.close()


@app.get("/entries")
async def list_entries(limit: int = 50):
    conn = await asyncpg.connect(PG_DSN)
    try:
        rows = await conn.fetch("SELECT id, source, title, body, tags, metadata, created_at, updated_at FROM journal_entries ORDER BY created_at DESC LIMIT $1", limit)
        return {"entries": [dict(row) for row in rows]}
    finally:
        await conn.close()


@app.get("/health")
async def health():
    return {"status": "ok", "service": "journal"}

if __name__ == "__main__":
    import uvicorn
    uvicorn.run(app, host="0.0.0.0", port=8000)
