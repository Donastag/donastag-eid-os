...
export async function GET(request: NextRequest) {
  ...
  if (projectId) {
    url.search = `project_id=${encodeURIComponent(projectId)}&limit=50`
  } else {
    url.search = 'limit=50'
  }
  ...
  if (!res.ok) {
    return new Response(JSON.stringify({ error: 'Failed to load lessons' }), { status: res.status })
  }
...
export async function POST(request: NextRequest) {
  ...
  if (!res.ok) {
    return new Response(JSON.stringify({ error: 'Failed to add lesson' }), { status: res.status })
  }
